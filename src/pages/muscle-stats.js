...
const MuscleStatsPage = {
    ...
    getAIAnalysis(stats, profile) {
        ...
        if (goal.includes('estetic') || goal.includes('fisico')) {
            ...
            analysis += `<p><strong>🏋️ Para fuerza:</strong> Enfócate en progresar en los compuestos pesados (squat, bench, deadlift). Tu punto más débil limita tu fuerza general.</p>`;
        } else {
    ...
    renderProgressiveOverload(prs, profile) {
        const weight = profile.weight || 70;
        const mainLifts = [
            { id: 'bench-press', name: 'Press Banca', icon: '🏋️', target: weight * 1.25 },
    ...
    generateStrengthPlan() {
        ...
        routine.name = '🏋️ Plan de Fuerza (PRs)';
